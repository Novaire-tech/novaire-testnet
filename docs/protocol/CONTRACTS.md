...
- **Responsibilities:** Supplies underlying into the configured Blend pool (or holds it directly under a test-only admin-set rate), mints/burns SY shares against deposits/redemptions, and exposes `exchange_rate()` as the single source of truth other layers read.
- **Storage:** `Config` (admin, underlying, pool, reserve_index), exchange rate, total shares, per-holder balance and principal, allowances.
- **Core Functions:** `initialize(admin, underlying)`, `initialize_blend(admin, underlying, pool)`, `deposit(from, amount) -> shares`, `redeem(from, sy_amount) -> underlying`, `exchange_rate()`, `underlying()`, `accrued_yield(holder)`, `migrate_reserve_index(admin)` (recovers from a Blend reserve reindex), plus SEP-41 token surface on the SY share (`balance`, `total_supply`, `transfer`, `transfer_from`, `approve`, `allowance`).
