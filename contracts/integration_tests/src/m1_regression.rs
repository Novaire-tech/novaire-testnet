...
fn test_m1_sell_yt_matches_complement_price() {
    ...
    let actual_out = protocol
        .marketplace
        .swap_yt_for_underlying(&alice, &yt_amount, &0);
...
fn test_m1_buy_yt_matches_complement_price() {
    ...
    let actual_out = protocol
        .marketplace
        .swap_underlying_for_yt(&alice, &usdc_in, &0);
...
fn test_m1_no_virtual_pt_movement() {
    ...
    assert_eq!(
        pt_res_before, pt_res_after_buy,
        "PT reserves changed during buy YT!"
    );

    let yt_in = 2_000_000;
    let _ = protocol
        .marketplace
        .swap_yt_for_underlying(&alice, &yt_in, &0);

    let (pt_res_after_sell, _, _) = protocol.marketplace.get_reserves();
    assert_eq!(
        pt_res_before, pt_res_after_sell,
        "PT reserves changed during sell YT!"
    );
...
fn test_m1_round_trip_no_free_profit() {
    ...
    let _ = protocol
        .marketplace
        .swap_underlying_for_yt(&bob, &1_000_000, &0);
    let bob_yt = protocol.yt_token.balance(&bob);
    let _ = protocol
        .marketplace
        .swap_yt_for_underlying(&bob, &bob_yt, &0);
...
fn test_m1_lp_accounting_preserved() {
    ...
    std::println!(
        "Before swap_underlying_for_yt - pt: {}, u: {}, yt: {}",
        pt,
        u,
        yt
    );
    ...
    let _ = protocol
        .marketplace
        .swap_yt_for_underlying(&bob, &1_000_000, &0);
...
fn test_m1_sequential_swaps() {
    ...
    let _ = protocol
        .marketplace
        .swap_underlying_for_yt(&bob, &200_000, &0);
...
fn test_m1_exploit_reproduction_at_epoch_start() {
    ...
    assert_eq!(
        protocol.env.ledger().sequence(),
        crate::framework::CREATED_LEDGER
    );
    ...
    let pt_proceeds = protocol
        .marketplace
        .swap_pt_for_underlying(&bob, &bob_pt, &0);
    ...
    std::println!(
        "Reserves After PT Swap - PT: {}, U: {}, YT: {}",
        pt_after_pt_swap,
        u_after_pt_swap,
        yt_after_pt_swap
    );
    ...
    std::println!(
        "Ledger before swap YT: {}",
        protocol.env.ledger().sequence()
    );
    let yt_proceeds = protocol
        .marketplace
        .swap_yt_for_underlying(&bob, &bob_yt, &0);
    ...
    std::println!(
        "Reserves After YT Swap - PT: {}, U: {}, YT: {}",
        pt_after_yt_swap,
        u_after_yt_swap,
        yt_after_yt_swap
    );
...
fn test_m1_repeat_attack() {
    ...
    let _ = protocol
        .marketplace
        .swap_pt_for_underlying(&bob, &bob_pt_2, &0);
    let _ = protocol
        .marketplace
        .swap_yt_for_underlying(&bob, &bob_yt_2, &0);
...
fn test_m1_large_capital_attack() {
    ...
    assert!(
        pnl <= 0,
        "Large capital attack is profitable! Net P&L: {}",
        pnl
    );
}
