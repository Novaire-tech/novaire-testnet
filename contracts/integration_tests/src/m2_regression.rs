...
fn test_m2_mint_at_genesis() {
    ...
    assert_eq!(
        claimable, 0,
        "No historical yield should be credited at genesis"
    );
...
fn test_m2_late_mint() {
    ...
    assert_eq!(
        bob_claimable, expected_historical,
        "Historical credit must exactly equal lost value"
    );
...
fn test_m2_multiple_late_minters() {
    ...
    assert_eq!(
        p2_claimable, expected_p2,
        "P2 should receive exact historical credit for current rate"
    );
...
fn test_m2_existing_holder_double_mint() {
    ...
    assert_eq!(
        claimable_after_second, expected,
        "Alice should have exact yield with no double counting"
    );
    ...
    assert_eq!(
        claimable_final, expected_final,
        "Alice should earn correct future yield on full balance"
    );
...
fn test_m2_no_trapped_shares() {
    ...
    assert!(
        tokenizer_shares < 10,
        "No vault shares should be permanently trapped in Tokenizer (except minimal rounding dust)"
    );
...
fn test_m2_claim_mint_claim() {
    ...
    assert_eq!(
        claimable_after_claim, 0,
        "Claimable yield should be cleared"
    );
...
fn test_m2_repeated_claim_no_remint() {
    ...
    assert!(
        claim_2 > 0,
        "Alice should claim again with no re-mint in between"
    );
    InvariantEngine::assert_everything(&protocol);
}
...
fn test_m2_multiple_refreshes_before_mint() {
    ...
    assert!(
        (bob_claimable - expected_historical).abs() <= 5,
        "Historical credit must use the latest exchange rate"
    );
    ...
    assert!(
        (bob_final_claimable - expected_total).abs() <= 5,
        "Total returned value must equal historical + future"
    );
...
fn test_m2_minimal_mint_rounding() {
    ...
    assert_eq!(
        claimable, expected,
        "Minimal mint should not underflow or crash"
    );
...
fn test_m2_large_value_stress() {
    ...
    assert!(
        (claimable - expected).abs() <= 5,
        "Large value math must not overflow"
    );
    ...
    assert!(
        diff <= 10,
        "Claimed amount should match expected total within rounding limits"
    );
    ...
    assert!(
        tokenizer_shares < 10,
        "No vault shares should be permanently trapped in Tokenizer"
    );
