...
fn test_m3_normal_settlement() {
    ...
    assert_eq!(
        ...
        "Rate should be stable after harvest"
    );
...
fn test_m3_attacker_early_settlement() {
    ...
    assert!(
        refreshed_rate > stale_rate,
        "Settlement must refresh rate and capture yield"
    );
    ...
    assert_eq!(
        refreshed_rate, final_rate,
        "Yield was already captured by settle refresh"
    );
    InvariantEngine::assert_everything(&protocol);
}
...
fn test_m3_h5_clamp_preserved() {
    ...
    assert!(
        rate_after <= max_rate,
        "H5 donation clamp must still be enforced during settlement refresh"
    );
    InvariantEngine::assert_everything(&protocol);
}
...
fn test_m3_repeated_settlement() {
    ...
    assert!(
        protocol.tokenizer.try_settle_epoch().is_err(),
...
fn test_m3_multiple_accruals() {
    ...
    assert!(
        rate > 1_000_000_000,
        "Must capture all accruals clamped properly"
    );
}
...
fn test_m3_original_attack_prevented() {
    ...
    assert!(
        claimable > 0,
        "YT holders must receive the final yield chunk"
    );
    InvariantEngine::assert_everything(&protocol);
}
