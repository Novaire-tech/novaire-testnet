...
fn test_m5_first_liquidity_mint() {
    ...
    assert!(
        protocol
            .marketplace
            .try_add_liquidity(&provider, &1000, &1000)
            .is_err(),
        "Should fail if <= MINIMUM_LIQUIDITY"
    );
    ...
    assert_eq!(
        lp_shares,
        expected_total - min_liquidity,
        "User LP must subtract MINIMUM_LIQUIDITY"
    );
...
fn test_m5_locked_liquidity_permanent() {
    ...
    assert!(
        under_res > 0,
        "Pool must have permanent underlying reserves locked"
    );
...
fn test_m5_second_liquidity_provider() {
    ...
    assert_eq!(
        lp2, 1_000_000,
        "Second provider gets full proportional shares, no min deduction"
    );
...
fn test_m5_donation_attack_impossible() {
    ...
    assert_eq!(
        lp_vic, 10_000_000,
        "Donations are ignored by storage reserves, LP minted proportionally"
    );
