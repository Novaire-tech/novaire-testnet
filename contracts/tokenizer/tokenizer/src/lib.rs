...
pub trait SyWrapperInterface {
    ...
    /// LOSS-02 / C-1: realizes any pending loss in the yield source against real
    /// on-chain backing. Must be called before `refresh_rate`/`get_exchange_rate`
    /// when freezing a settlement rate, since `refresh_rate` can only ever
    /// ratchet `TotalUnderlying` up and silently leaves a stale, pre-loss rate
    /// frozen forever otherwise. See `SyWrapper::mark_loss` for why this is safe
    /// to call permissionlessly (it can only ever push the recorded balance
    /// toward on-chain ground truth, never away from it).
...
impl Tokenizer {
    ...
    pub fn settle_epoch(env: Env) -> Result<(), NovaireTokenizerError> {
        ...
        // LOSS-02 / C-1: realize any pending loss against real on-chain backing FIRST, in
        // the same transaction, before freezing the settlement rate. `refresh_rate` alone
        // (Fix M3) only ever ratchets `TotalUnderlying` up, so if a Blend loss happened and
        // nobody has called `mark_loss` yet, `refresh_rate` would do nothing and the
        // settlement rate would freeze the stale pre-loss backing, letting early PT
        // redeemers extract more than actually exists. `mark_loss` is a no-op when there is
        // no loss (see its doc comment in sy_wrapper), so this has no effect on the
